...
import com.qst.utils.{FastJsonUtils, HbaseUtils, HdfsPropertiesUtils}
...
object KafkaToHbaseWork {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setAppName("qst_work")
    conf.set("spark.testing.memory", "2147480000") // 后面的值大于512m即可
    val ssc = new StreamingContext(conf, Seconds(4))

    //get configuration
    val kafkaParams = Map("metadata.broker.list" -> prop.getProperty("metadata.broker.list"), "group.id" -> prop.getProperty("group"))
